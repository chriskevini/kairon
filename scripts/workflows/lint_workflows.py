...
def check_postgres_node_pattern(node: dict, result: LintResult):
    """Check if Postgres nodes use ctx for query parameters or suggest Execute_Query"""
    name = node.get("name", "Unknown")
    options = node.get("parameters", {}).get("options", {})
    query_replacement = options.get("queryReplacement", "")

    # Suggest using Execute_Query workflow for complex queries
    result.warn(
        f"'{name}': Consider using 'Execute_Query' sub-workflow to ensure ctx preservation"
    )
