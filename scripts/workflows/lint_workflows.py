...
def check_workflow_structure(workflow: dict, result: LintResult):
    """Check for broken connections and orphan nodes (structural validation)"""
    ...
    for src, outputs in conns.items():
        if src not in nodes:
            result.error(f"Connection from non-existent node: {src}")
    ...
    for node in workflow.get("nodes", []):
        ...
        if "trigger" not in node_type and node["name"] not in connected:
            result.warn(f"Orphan node (not connected): {node['name']}")
