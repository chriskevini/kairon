...
class TestRoute_Message:
    ...
    def test_entry_point(self):
        ...
        assert triggers[0]["name"] == "Receive Event"

    def test_tag_routing_rules(self):
        """Test that all required tags are routed"""
        workflow = load_workflow()
        nodes = workflow.get("nodes", [])
        route_node = next((n for n in nodes if n["name"] == "Route by Tag"), None)
    ...
    def test_projection_preparation(self):
        ...
        workflow = load_workflow()
        nodes = workflow.get("nodes", [])
        prep_node = next((n for n in nodes if n["name"] == "Prepare Projection"), None)
    ...
    def test_subworkflow_calls(self):
        ...
        assert "Multi Capture" in subworkflows
        assert "Capture Projection" in subworkflows
        assert "Execute Command" in subworkflows
        assert "Start Thread" in subworkflows
